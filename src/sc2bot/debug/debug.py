...
from sc2bot.debug.scenario import MicroScenario
...
class Debug:
    ...
    micro_scenario: Optional[MicroScenario]
    ...
    async def on_step_start(self, step: int) -> None:
        ...
        if self.micro_scenario is not None:
            if await self.micro_scenario.step():
        ...
        for chat_message in self.bot.state.chat:
            ...
            if chat_message.message.startswith('!'):
                ...
                if cmd == '!debug' and len(args) == 1 and args[0] in {'0', '1'}:
                    ...
                    await self.client.debug_show_map()
                    self.micro_scenario = MicroScenario(self.bot, units={UnitTypeId.MARINE: 8})
