...
class UnitCountTask(Task):
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(utype={self.utype.name}, number={self.number}, position={self.position})"
...
class OrderTask(Task):
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(target={self.target}, units={self.units})"
...
class TaskManager:
    ...
    def dependencies_fulfilled(self, dependencies: dict[int, TaskStatus]) -> bool:
    ...
    async def update_tasks(self, iteration: int) -> None:
        ...
        for task in self.future.copy().values():
            if self.dependencies_fulfilled(task.deps):
